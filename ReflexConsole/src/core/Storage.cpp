...
bool Storage::begin(Stats&s){if(!prefs.begin("reflex",false)){DEBUGF("Preferences open failed\n");return false;} settings.sound=prefs.getBool("sound",true);settings.led=prefs.getBool("led",true);settings.length=prefs.getUChar("length",1);settings.quickTrials=prefs.getUChar("trials",10);settings.lapseMs=prefs.getUShort("lapse",500);s.sessions=prefs.getULong("sessions",0);s.personalBest=prefs.getUShort("best",0);s.memoryBest=prefs.getUChar("memBest",0);s.scoreCount=prefs.getUChar("scoreN",0);prefs.getBytes("scores",s.scores,sizeof(s.scores));prefs.getBytes("last",&s.last,sizeof(s.last));s.baseline.quickSamples=prefs.getUChar("qN",0);s.baseline.quickMedian=prefs.getFloat("qMed",0);s.baseline.quickSpread=prefs.getFloat("qDev",0);s.baseline.quickLapseRate=prefs.getFloat("qLap",0);s.baseline.focusMedian=prefs.getFloat("fMed",0);s.baseline.focusLapseRate=prefs.getFloat("fLap",0);s.baseline.rhythmError=prefs.getFloat("rErr",0);
...
const char* Storage::testKindName(uint8_t kind){switch(static_cast<TestKind>(kind)){case TestKind::QUICK:return "quick";case TestKind::FOCUS:return "focus";case TestKind::CHOICE:return "choice";case TestKind::RHYTHM:return "rhythm";case TestKind::MEMORY:return "memory";}return "unknown";}
