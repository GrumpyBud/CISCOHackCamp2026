...
export async function GET() {
  ...
  const rows = await sql`
    SELECT d.badge_id, s.sequence, s.test_type, s.score, s.median_ms, s.spread_ms,
           ...
           h.sleep_hours, h.sleep_quality, h.stress, h.mood, h.exercise_minutes,
           h.caffeine_mg, h.hydration, h.notes AS health_notes
    FROM sessions s JOIN devices d ON d.id = s.device_id
    LEFT JOIN health_logs h ON h.clerk_user_id = s.clerk_user_id AND h.log_date = s.imported_at::date
  ...
  ` as unknown as {
    ...
    exercise_minutes: number | null; caffeine_mg: number | null; hydration: number | null; health_notes: string | null;
  }[];
  const headers = ["badge_id", "sequence", "test_type", "score", "median_ms", "spread_ms", "lapses", "false_starts", "attempts", "correct", "rhythm_bias_ms", "imported_at", "sleep_hours", "sleep_quality", "stress", "mood", "exercise_minutes", "caffeine_mg", "hydration", "health_notes"];
